 ...
 */
public class ItemRequestItemViewAdapter extends ArrayAdapter<ItemRequest> {
    ...
    public ItemRequestItemViewAdapter(Context c, ArrayList<ItemRequest> itemRequests, View.OnClickListener listenerDecline, View.OnClickListener listenerAccept ){
        super(c,  R.layout.list_row_item_request_item_view, R.id.friend_name, itemRequests);
        this.context = c;
    ...
    public View getView(int position, View convertView, ViewGroup parent){
        ...
        View row = inflater.inflate(R.layout.list_row_item_request_item_view, parent, false);
        ...
        TextView id = (TextView)row.findViewById(R.id.item_request_item_view_id);
        TextView pos = (TextView)row.findViewById(R.id.item_request_position_item_view_id);

        //getItemImage(this.itemRequests.get(position).getItemID());

        if(this.itemRequests.get(position).getFromImage().equals("")){
            ...
            BlobImageLoaderUtil imageLoader = new BlobImageLoaderUtil();
            imageLoader.loadImage(this.itemRequests.get(position).getFromImage(), image, 550);
        }
        ...
        if(this.itemRequests.get(position).getFromName().length() > 12) {
            name.setText(this.itemRequests.get(position).getFromName().substring(0, 11) + "...");
        }
        else{
            name.setText(this.itemRequests.get(position).getFromName());
        }
